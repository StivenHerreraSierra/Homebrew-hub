...
export class DetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(nombrePaquete) {
      this.homebrewService.getPaquete(nombrePaquete)
        .subscribe((data: Paquete) => this.paquete = data);
  ...
  cargarChartMac() {
    const valoresMac = [
      Number(this.paquete['analytics-30'].replace(/,/g, '')),
      Number(this.paquete['analytics-90'].replace(/,/g, '')),
      Number(this.paquete['analytics-365'].replace(/,/g, ''))
    ];
  ...
  cargarChartLinux() {
    const valoresLinux = [
      Number(this.paquete['analytics-linux-30'].replace(/,/g, '')),
      Number(this.paquete['analytics-linux-90'].replace(/,/g, '')),
      Number(this.paquete['analytics-linux-365'].replace(/,/g, ''))
    ];
